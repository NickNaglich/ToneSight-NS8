...
[2026-03-02T01:10:45Z] Run release-readiness gate and prepare v0.2.7 distribution metadata
Files: pyproject.toml, src/tonesight_ns8/observability_api.py, README.md, docs/RELEASE_CHECKLIST.md, docs/RELEASE_NOTES_0.2.7.md, .agent/LOGS/CHANGE_LOG.md
Reason: Execute final in-repo release checklist and align package/distribution metadata for broad publication by bumping to 0.2.7, synchronizing observability version, documenting release notes, and updating checklist commands to current vector check/build/twine flow.
Validation: python tools/validate_defaults.py; python tools/validate_defaults.py tests/fixtures/defaults.invalid.json (expected failure); python tools/check_nosec_policy.py; python -m bandit -r src; python -m tonesight_ns8.cli release-check --goldset data/goldset.jsonl --taxonomy taxonomy/tone_taxonomy.v1.json (decision=passed); python tools/regen_vectors.py --check; python -m pytest -q (263 passed); python -m build; python -m twine check dist/tonesight_ns8-0.2.7*
[2026-03-03T19:52:22Z] Implement hybrid drop-in packaged assets with runtime fallback path resolution
Files: src/tonesight_ns8/defaults.py, src/tonesight_ns8/cli.py, src/tonesight_ns8/gate_runner.py, src/tonesight_ns8/release_check_runner.py, src/tonesight_ns8/package_data/config/gate_profiles.json, src/tonesight_ns8/package_data/data/goldset.jsonl, src/tonesight_ns8/package_data/taxonomy/tone_taxonomy.v1.json, src/tonesight_ns8/package_data/vectors/ns8_test_vectors.json, pyproject.toml, tests/test_defaults_config.py, README.md, .agent/LOGS/CHANGE_LOG.md
Reason: Make tonesight-ns8 usable out-of-the-box after install by shipping deterministic built-in assets and automatically falling back when repo-relative defaults are unavailable in downstream projects.
Validation: python -m pytest -q tests/test_defaults_config.py test_cli.py tests/test_release_check_runner.py (44 passed); python -m build; python -m twine check dist/*
