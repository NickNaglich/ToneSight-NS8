"""Validation helpers for canonical LiveEvent JSONL envelopes."""
...
def validate_live_event(payload: dict[str, Any], *, row_num: int) -> None:
...
def validate_live_event_file(path: Path) -> None:
    ...
    for row_num, raw in enumerate(lines, start=1):
        ...
        validate_live_event(payload, row_num=row_num)
    if row_count == 0:
