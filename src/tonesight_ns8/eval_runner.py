...
def _render_eval_report_html(
    *,
    ...
    return (
        ...
        "<style>\n"
        ":root { --bg:#f3f6fb; --panel:#ffffff; --ink:#12243c; --muted:#51657f; --line:#ced9e8; --accent:#0b7e9f; --ok:#2f9e44; --warn:#c92a2a; }\n"
        "body { font-family: Segoe UI, Trebuchet MS, sans-serif; margin: 0; background: radial-gradient(circle at 12% 12%, #e0e9f7 0%, #f6f9ff 55%, #fbfdff 100%); color: var(--ink); }\n"
        ...
        ".kpi { background:#f9fbff; border:1px solid var(--line); border-radius:8px; padding:8px; }\n"
        ...
        "canvas { width: 100%; height: 420px; background: #f7fbff; border: 1px solid var(--line); border-radius: 8px; }\n"
        ".grid3 { display:grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap:10px; }\n"
        ".grid5 { display:grid; grid-template-columns: repeat(5,minmax(0,1fr)); gap:10px; }\n"
        "svg { width: 100%; height: 180px; background:#f7fbff; border:1px solid var(--line); border-radius:8px; }\n"
        ...
        "th, td { border: 1px solid var(--line); padding: 6px; text-align: left; font-size: 12px; }\n"
        ...
        '<label>Max rows<input id="maxRows" type="range" min="1" max="1" value="1" /></label><span id="maxRowsVal" class="muted"></span><span id="pointStats" class="muted" style="margin-left:8px;"></span>'
